 ...
 * it.
 */
client_t *client_create(uint16_t id, uint32_t sourceid,socket_t *tcp_sock, socket_t *udp_sock,
                        int connected)
{
    client_t *c = NULL;
...
int client_recv_udp_msg(client_t *client, char *data, int data_len,
                        uint16_t *id, uint8_t *msg_type, uint16_t *len,uint32_t *sourceid)
{
    int ret;
    

    ret = msg_recv_msg(client->udp_sock,data, data_len,
                       id, msg_type, len, sourceid);
    if(ret < 0)
        return ret;
                        ...
                        uint8_t msg_type)
{
	...
	char *enbuf;
	enbuf=desdecrypt(data, data_len,client->deskey);
    ...
    {
        memcpy(client->udp2tcp, enbuf, data_len);
...
int client_recv_tcp_data(client_t *client)
{
    ...
    ret = sock_recv(client->tcp_sock, buf.buf, sizeof(buf.buf));
    if(ret < 0)
        return -1;
    if(ret == 0)
        return -2;
...
int client_send_udp_data(client_t *client)
{
	...
	char *enbuf=desencrypt(buf->buf, buf->len,client->deskey);
	int data_rest = buf->len % 8;  
	int len;
	if (data_rest==0)
		len=buf->len;
	else
		len = buf->len + (8 - data_rest);		
	//printf("encrypt key is %s datalen is %d padded is %d \n",client->deskey,buf->len,len);
    ret = msg_send_msg(client->udp_sock, client->id,client->sourceid, msg_type,
                       enbuf, len);
    if(ret < 0)
        return ret;
 ...
 */
int client_socks5_helloack(client_t *client)
{
	...
	send(client->tcp_sock->fd,socks5_response,10,0);
	return 0;
}
int client_send_hello(client_t *client, char *host,
                      uint16_t req_id)
{
    return msg_send_hello(client->udp_sock, host,req_id,client->sourceid,client->rsakey);
}

int client_send_reg(client_t *client, char *host, char *port,
                      uint16_t req_id)
{
    return msg_send_reg(client->tcp_sock, host, port, req_id,client->sourceid);
}

int client_send_req(client_t *client, char *host, char *port,
                      uint16_t req_id)
{
    return msg_send_req(client->tcp_sock, host, port, req_id,client->sourceid);
}
...
int client_send_helloack(client_t *client, uint16_t req_id)
{
	char * key=malloc(11);
	memset(key,0,11);
    if (client->deskey[0]==0)
	{		
		sprintf(key,"%d",(uint32_t)rand());
		printf("key is %s \n",key);
		return msg_send_helloack(client->udp_sock,key,req_id,client->id,client->sourceid,client->rsakey);
	}
	else
		return msg_send_helloack(client->udp_sock,&client->deskey,req_id,client->id,client->sourceid,client->rsakey);
	
	
    //return msg_send_msg(client->udp_sock, client->id, client->sourceid,MSG_TYPE_HELLOACK,
    //                   (char *)&req_id, sizeof(req_id));

	
	
	if(debug_level >= DEBUG_LEVEL3)
		printf("req_id %d id %d, sourceid %d\n",req_id,client->id,client->sourceid);
	
	
}

int client_send_regack(client_t *client, uint16_t req_id)
{
    req_id = htons(req_id);
	
    return msg_send_msg(client->tcp_sock, req_id, client->sourceid,MSG_TYPE_REGACK,
                        (char *)&req_id, sizeof(req_id));
	if(debug_level >= DEBUG_LEVEL3)
		printf("send reg ackreq_id %d \n",req_id);
}
...
int client_got_regack(client_t *client)
{
	if(debug_level >= DEBUG_LEVEL3)
		printf("got reg ack \n");
    if(client->udp2tcp_state == CLIENT_WAIT_HELLO)
...
int client_got_reqack(client_t *client)
{
	if(debug_level >= DEBUG_LEVEL3)
		printf("got req ack \n");
    if(client->udp2tcp_state == CLIENT_WAIT_HELLO)
...
{
    return msg_send_msg(client->udp_sock, client->id, client->sourceid,MSG_TYPE_GOODBYE,
                        NULL, 0);
}
...
int client_check_and_send_keepalive(client_t *client, struct timeval curr_tv)
{
    if(client_timed_out(client, curr_tv))
    {
        ...
        return msg_send_msg(client->udp_sock, client->id, client->sourceid,MSG_TYPE_KEEPALIVE,
                            NULL, 0);
    }
